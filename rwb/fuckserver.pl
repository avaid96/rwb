...
if ($action eq "base") { 
  ...
  #DYNAMIC CYCLES
  print "<p> Available cycles (dynamic): ";
  my @cycles;
  eval { 
    @cycles = ExecSQL($dbuser, $dbpasswd, "select distinct cycle from cs339.committee_master",undef);
  };

  #my @cycArr = $cycles[1][0];
  foreach my $el (@cycles) {
	foreach my $val (@$el) {
	print "$val, ";
	}
  } 
  ...
  print "<p>Enter cycle numbers here. EG:\"1111,2222\"</p>";
  print "<input type=\"text\" id=\"lname\"><br>";
  print "<p>Enter what you want. EG:\"committees,candidates\"</p>";
  print "<input type=\"text\" id=\"wname\"><br>";

  #SUMMARIES
  print "<h5>COMMITTEE SUMMARY WILL COME HERE WHEN SELECTED</h5>";
  print "<div id=\"committeeSum\" style=\:width:100\%; height:10\%\"></div>";
  print "<h5>INDIVIDUAL SUMMARY WILL COME HERE WHEN SELECTED</h5>";
  print "<div id=\"individualSum\" style=\:width:100\%; height:10\%\"></div>";
  print "<h5>OPINION SUMMARY WILL COME HERE WHEN SELECTED</h5>";
  print "<div id=\"opinionSum\" style=\:width:100\%; height:10\%\"></div>";
  #---------
...
sub Opinions {
  if (!UserCan($user,"query-opinion-data")) { 
	print "<p id=\"no-opin-perm\">You don't have permission to query this data, sorry!</p>";
  }
  else {
  my ($latne, $longne, $latsw, $longsw, $cycle,$format) = @_;
  my @rows;
  ...
  my @stat;
  eval { @stat = ExecSQL($dbuser, $dbpasswd, "select avg(color), stddev(color), count(color) from rwb_opinions where latitude>? and latitude<? and longitude>? and longitude<?",undef,$latsw,$latne,$longsw,$longne); };
  my $statHolder = @stat[0];
  my $avg = @{$statHolder}[0];
  my $stddev = @{$statHolder}[1];
  my $numOpin = @{$statHolder}[2];

  my $numIt = 0;

  while ($numOpin < $minSum && $numIt < 7) {
        $latne += 0.1;
        $latsw -= 0.1;
        $longne += 0.1;
        $longsw -= 0.1;
  ...
  my $finalRes = "<div id=\"opinSum\">COUNT (number of opinions): $numOpin, AVG: $avg, STDDEV: $stddev [Note: where D=-1 and R=1]";
  print $finalRes;
  print "<div id=\"opSumClr\">$color</div>";
